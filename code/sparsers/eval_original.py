...
class TSRDModel():
    def __init__(self):
        ...
        self.model = nn.DataParallel(model_pt.cuda())
        self.model.eval()
        self.mu = torch.Tensor([0.4285, 0.4162, 0.4299]).float().view(1, 3, 1, 1).cuda()
        self.sigma = torch.Tensor([0.2566, 0.2509, 0.2700]).float().view(1, 3, 1, 1).cuda()
...
if __name__ == '__main__':
    ...
    if args.dataset == 'ImageNet':
        ...
        test_loader = data.DataLoader(imagenet, batch_size=args.bs, shuffle=False, num_workers=0)

        testiter = iter(test_loader)
        x_test, y_test = next(testiter)
# ----------------------------------------------------------------------------------------------------------------------
    elif args.dataset == 'masked_tsrd':
        # load pretrained model
        model = TSRDModel()
        assert not model.model.training
        ...
        # load data
        TSRD_SL = 224
        TSRD_PATH = args.data_path
        tsrd = datasets.ImageFolder(TSRD_PATH,
                                        transforms.Compose([
                                            transforms.Resize(TSRD_SL),
                                            transforms.CenterCrop(TSRD_SL),
                                            transforms.ToTensor()
                                        ]))
        torch.manual_seed(0)

        test_loader = data.DataLoader(tsrd, batch_size=args.bs, shuffle=False, num_workers=0)

        testiter = iter(test_loader)
        x_test, y_test = next(testiter)

        # TODO any possible path here
        MASKS_PATH = os.path.join(os.getcwd(), "../../datasets/tsrd_mask/testmask")
        masks = torch.zeros([x_test.shape[0], 3, 224, 224]).cpu()
        for i in range(x_test.shape[0]):
            sample_fname, _ = test_loader.dataset.samples[i]
            imname = os.path.basename(sample_fname)
            impath = os.path.join(MASKS_PATH, imname)
            pilimage = Image.open(impath).resize([224, 224])
            masks[i] = torch.from_numpy(np.array(pilimage)).reshape([1, 224, 224]).repeat_interleave(3, dim=0)
# ----------------------------------------------------------------------------------------------------------------------
    ...
    if args.attack in ['rs_attack']:
        ...
        with torch.no_grad():
            ...
            for counter in range(n_batches):
                ...
                if args.dataset == 'masked_tsrd':
                    mask_curr = masks[ind_to_fool[counter * bs:(counter + 1) * bs]].cuda()
                # qr_curr is the necessary queries for the samples (0 if not successful)
                # adv are the resulting adversarial images complete with perturbation
                qr_curr, unmaskedadv = adversary.perturb(x_curr, y_curr, mask_curr if args.dataset == 'masked_tsrd' else None)

                # Mask solutions
                if args.dataset == 'masked_tsrd':
                    adv = x_curr.clone()
                    adv[mask_curr > 0] = unmaskedadv[mask_curr > 0]
                else:
                    adv = unmaskedadv
