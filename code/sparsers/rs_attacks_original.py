...
class RSAttack():
    ...
    def margin_and_loss(self, x, y):
        ...
        # Image.fromarray((x[0].cpu().numpy()*255.0).transpose([1,2,0]).astype(np.uint8)).show()
    ...
    def init_hyperparam(self, x):
        assert self.norm in ['L0', 'patches', 'frames', 'masked_patches', 'masked_scratches',
                             'patches_universal', 'frames_universal', 'scratches']
    ...
    def attack_single_run(self, x, y, masks=None):
        ...
        with torch.no_grad():
            ...
            # ----------------------------------------------------------------------------------------------------------
            if self.norm == 'scratches':
                ''' assumes square images and patches '''
                # TODO allow any scratch count and length
                scratch_count = 3  # 3
                scratch_length = 130  # 130
                attack = attackmodel.TorchBezierAttack(h, ell0=scratch_length, width=1, linecount=scratch_count)
                opt = optimizer.SparseRSPSO(attack.searchspace, x.shape[0])
                opt.initstep()
                ...
                for it in range(1, self.n_queries):
                    ...
                    x_new = x_curr.clone()
                    suggested_solutions = opt.ask(idx_to_fool)
                    x_new = attack(x_new, suggested_solutions).clamp(0., 1.)

                    # Image.fromarray((np.array(x_new[0].cpu())*255).transpose([1,2,0]).astype(np.uint8)).show()

                    # check loss of new candidate
                    margin, loss = self.margin_and_loss(x_new, y_curr)

                    n_queries[idx_to_fool] += 1
                    opt.tell(idx_to_fool, loss)
                ...
                # Image.fromarray((np.array(patches_coll[0].cpu())*255).transpose([1,2,0]).astype(np.uint8)).show()
                # exit()

                x_masked = x.clone()
                x_masked[masks > 0] = x_new[masks > 0]

                margin_min, loss_min = self.margin_and_loss(x_masked, y)
                n_queries = torch.ones(x.shape[0]).to(self.device)
                ...
                for it in range(1, self.n_queries):
                    ...
                    # Image.fromarray((np.array(patches_curr[0].cpu())*255).transpose([1,2,0]).astype(np.uint8)).show()
                    ...
                    # sample update
                    # p_selection gives p_init/schedule parameter. p_init is alpha_0, s_it is s_i
                    s_it = int(max(self.p_selection(it) ** .5 * s, 1))
                    # p_it is the updated square location
                    p_it = torch.randint(s - s_it + 1, size=[2])
                    # sh_it is the shift to apply to the patch
                    ...
                    # Mask candidates
                    x_masked = self.check_shape(x[idx_to_fool])
                    x_masked[masks[idx_to_fool] > 0] = x_new[masks[idx_to_fool] > 0]
                    ...
                    if nimpr > 0.:
                        ...
                        x_best[idx_to_fool[idx_improved]] = x_masked[idx_improved].clone()
            ...
            elif self.norm == 'masked_scratches':
                ''' assumes square images and patches '''
                # TODO allow any scratch count and length
                scratch_count = 3  # 3
                scratch_length = 130  # 130
                ...
                for it in range(1, self.n_queries):
                    ...
                    # check loss of new candidate
                    margin, loss = self.margin_and_loss(x_masked, y_curr)
                ...
                for it in range(1, self.n_queries):
                    ...
                    if self.verbose:
                        if ind_succ.numel() != 0:
                            ...
                            self.logger.log('no success at iteration ' + str(it) + ' min loss: ' + str(loss_min.min()))
                    if ind_succ.numel() == n_ex_total:
                        break
    ...
    def perturb(self, x, y=None, masks=None):
        """
        ...
        for counter in range(self.n_restarts):
            ...
            if ind_to_fool.numel() != 0:
                ...
                qr_curr, adv_curr = self.attack_single_run(x_to_fool, y_to_fool, masks)
